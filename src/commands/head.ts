...
export const head: Command = {
	...
	async execute(args: string[], ctx: CommandContext): Promise<CommandResult> {
		...
		for (let i = 0; i < args.length; i++) {
			...
			if (arg === '-n' && i + 1 < args.length) {
				...
				if (val.startsWith('-')) {
					...
					lineCount = Number.parseInt(val, 10);
				}
				continue;
			}
			...
			for (let i = 0; i < files.length; i++) {
				if (files[i] === '-') {
					...
					try {
						...
						stderr += `head: cannot open '${files[i]}' for reading: No such file or directory\n`;
					}
				}
			}
		}
		...
		for (let i = 0; i < inputs.length; i++) {
			...
			if (byteCount >= 0) {
				stdout += content.slice(0, byteCount);
			} else {
				const lines = content.split('\n');
				let count: number;
				if (negativeLines) {
					count = Math.max(0, lines.length - lineCount);
					// Adjust for trailing empty element
					if (content.length > 0 && content[content.length - 1] === '\n') {
						count = Math.max(0, lines.length - 1 - lineCount);
					}
				} else {
					count = lineCount;
				}
